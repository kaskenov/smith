...
import { isValidTemplateName } from '../core/templateName';
...
/** List real template directories only — symlink entries and invalid names are skipped. */
export function listTemplateNamesInDir(templatesDir: string): string[] {
  if (!existsSync(templatesDir)) return [];
  return readdirSync(templatesDir)
    .filter((entry) => {
      if (!isValidTemplateName(entry)) return false;
