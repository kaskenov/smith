...
description: >-
  ...
  functions, presets, and hook execution order. Use when the user edits .smith/config.js,
...
| `defaultPreset` | string | Preset name used when `--preset` is omitted |
| `presets` | `Record<string, preset>` | Named include/exclude pattern sets |
| `before` | async hook | Runs before file replication |
| `after` | async hook | Runs after file replication |

Each preset is `{ include?: string[], exclude?: string[] }` with glob path patterns matched against template relative paths.
...
1. CLI/MCP `--preset` / `preset`
2. `defaultPreset` from merged config
...
Patterns match template paths before placeholder substitution, e.g. `{{name}}.vue`, `**/*.spec.ts`.
...
- Validates preset shape and `defaultPreset` references
- Throws when config is invalid
...
| `defaultPreset` | Template wins if set |
| `presets` | Template wins if set |
